# 네이버 날씨 홈 위젯 플러그인 (v2.2.0)

## v2.2.0 — 탭별 표시/숨김 체크박스 추가

환경설정 > 플러그인 설정 맨 위에 4개 체크박스가 추가되었습니다:

- `SHOW_TAB_GENERAL` — [일반] 탭에 표시
- `SHOW_TAB_ADULT` — [성인] 탭에 표시
- `SHOW_TAB_AUDIOBOOK` — [오디오] 탭에 표시
- `SHOW_TAB_VIDEO` — [영상] 탭에 표시

기본값은 전부 켜짐(true)이며, 체크 해제한 탭에서는 위젯이 네이버에 요청을 보내지도 않고
곧바로 "이 탭에서는 꺼져 있습니다" 안내만 표시합니다(사실상 숨김 처리).

**참고**: 위젯 자체는 "+ 위젯 추가" 목록에서는 4개 탭 모두에 계속 나타납니다(선택 가능한
상태 자체를 탭별로 완전히 감추는 것은 플러그인 매니페스트 구조상 서버 기동 시점에 결정되는
값이라 설정값으로 안전하게 바꿀 수 없습니다). 대신 체크 해제한 탭에서 위젯을 추가해 보면
실제 날씨 데이터 대신 위 안내 문구만 나타나므로, 사실상 그 탭에서는 아무 정보도 노출되지
않습니다.
